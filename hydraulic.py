...
def pressure_balance(G: float, T: list[float]):
    ...
    p_loses += dp_expansion(G, T[i], dt.f_1, dt.f_2)
    ...
    for _ in range(dt.n_3):
        ...
        i += 1
    p_loses += dp_bend_90(G / 4, T[i], dt.f_3)
    p_loses += dp_az_pg_in_and_out(G / 4, T[i], 0.5)
    dx = dt.h_pg / dt.n_pg
    ...
    p_loses += dp_bend_90(G / 4, T[i], dt.f_4)
    ...
    p_loses += dp_bend_90(G / 4, T[i], dt.f_5)

    for _ in range(dt.n_5):
        p_loses += dp_friction(G / 4, T[i], dx, dt.f_5)
        i += 1

    p_loses += dp_bend_90(G / 12, T[i], dt.f_5)
    ...
    for _ in range(dt.n_6):
        ...
        i += 1
    p_loses += dp_bend_90(G / 12, T[i], dt.f_6)
    # Горизонталь до АЗ
    dx = dt.l_7 / dt.n_7
    for _ in range(dt.n_7):
        p_loses += dp_friction(G / 4, T[i], dx, dt.f_7)
        i += 1
    p_loses += dp_bend_90(G / 4, T[i], dt.f_7)
